...
class JiraServerAdapterTest {
    ...
    void should_returnTrue_when_testConnectionSucceeds() {
        configureAdapter();
        setupGetMock();
        when(responseSpec.bodyToMono(String.class)).thenReturn(Mono.just("{\"name\":\"admin\"}"));
    ...
    void should_fetchTasks_when_validProjectKey() {
        ...
        String jsonResponse = """
                {
                  "issues": [
                    ...
                    {
                      "key": "PROJ-2",
                      "self": "https://jira.mycompany.com/rest/api/2/issue/10002",
                      "fields": {
        ...
        assertEquals("https://jira.mycompany.com/browse/PROJ-1", task1.getExternalUrl());
        assertEquals("Fix the login bug", task1.getTitle());
        assertEquals("Bug description text", task1.getDescription());
    ...
    @Test
    @SuppressWarnings("unchecked")
    void should_fetchTasks_with_statusAndAssigneeFilter() {
        configureAdapter();
        setupGetMock();
    ...
    void should_getTask_when_validExternalId() {
        ...
        String jsonResponse = """
                {
                  "key": "PROJ-42",
                  "self": "https://jira.mycompany.com/rest/api/2/issue/10042",
                  "fields": {
                    "summary": "Implement auth",
                    "description": "Auth description in plain text",
                    "status": {"name": "Done"},
        ...
        assertEquals("https://jira.mycompany.com/browse/PROJ-42", task.getExternalUrl());
        assertEquals("Implement auth", task.getTitle());
        assertEquals("Auth description in plain text", task.getDescription());
