...
class QAAgentServiceTest {
    ...
    void should_executeQA_successfully() {
        ...
        when(conversationService.startConversation(tenantId, taskId, userId, "QA")).thenReturn(conversation);
    ...
    void should_executeQA_withFailVerdict() {
        ...
        when(conversationService.startConversation(tenantId, taskId, userId, "QA")).thenReturn(createConversation());
        when(configService.resolveAgentConfig(tenantId, null, userId, "QA"))
    ...
    void should_parseQAVerdict_conditionalPass() {
        assertEquals("CONDITIONAL_PASS", qaAgentService.parseQAVerdict("[QA_CONDITIONAL_PASS] Some concerns."));
    }
    ...
    void should_parseQAVerdict_freeTextPass() {
        assertEquals("PASS", qaAgentService.parseQAVerdict("Overall QA verdict: PASS\n\nAll tests pass."));
    }

    @Test
    void should_parseQAVerdict_freeTextFail() {
        assertEquals("FAIL", qaAgentService.parseQAVerdict("Overall QA verdict: FAIL\n\nTests fail."));
    }
    ...
    void should_buildQAPromptWithTools() {
        List<ToolDefinition> tools = List.of(
                ToolDefinition.builder().name("shell_exec").description("Executes a shell command")
                        .parameters(List.of(ToolParameter.builder().name("command").type("string")
                                .description("The command to execute").required(true).build())).build()
        );

        String prompt = qaAgentService.buildQAPromptWithTools("Implement user authentication", null, tools);
    ...
    void should_buildQAPromptWithTools_withDiff() {
        String prompt = qaAgentService.buildQAPromptWithTools("Fix login bug",
                "diff --git a/Login.java b/Login.java\n-old code\n+new code", Collections.emptyList());
        assertTrue(prompt.contains("diff --git a/Login.java b/Login.java"));
    }

    @Test
    void should_buildQAPromptWithTools_withoutDiff() {
        String prompt = qaAgentService.buildQAPromptWithTools("Test task", null, Collections.emptyList());
    ...
    private Conversation createConversation() {
        return Conversation.builder()
                ...
                .agentType("QA").status("ACTIVE").totalTokens(0L).build();
    }
}
